...
module.exports = class MonsterGame {
  ...
  listMonsters = () => {
      this.monsterList.forEach(monster => {
          console.log( "Monster: " + `${monster.monsterName}` + ", Minimum Life: " + `${monster.minimumLife}` + ", Current Life: " + `${monster.currentLife}` )
      });
  }
  ...
  async playGame(GameDelay) {
    ...
    do {
      ...
      this.monsters.forEach((monster) => {
          console.log("Monster is alive: " + monster.isAlive)
          if (monster.isAlive) {
            console.log("randDrain running")
